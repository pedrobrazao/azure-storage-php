...
class LokiBlobMetadataStore {
    ...
    async filterBlobs(context, account, container, where, maxResults = constants_1.DEFAULT_LIST_BLOBS_MAX_RESULTS, marker = "") {
        const query = {};
    ...
    async acquireBlobLease(context, account, container, blob, duration, proposedLeaseId, options = {}) {
        const coll = this.db.getCollection(this.BLOBS_COLLECTION);
        const doc = await this.getBlobWithLeaseUpdated(account, container, blob, undefined, context, false); // This may return an uncommitted blob, or undefined for an nonexistent blob
        (0, WriteConditionalHeadersValidator_1.validateWriteConditions)(context, options.modifiedAccessConditions, doc);
    ...
    async copyFromURL(context, source, destination, copySource, metadata, tier, options = {}) {
        ...
        const copiedBlob = {
            ...
            blobTags: options.copySourceTags === Models.BlobCopySourceTags.COPY ?
                sourceBlob.blobTags
                : options.blobTagsString === undefined ? undefined : (0, utils_3.getTagsFromString)(options.blobTagsString, context.contextId)
        };
        if (copiedBlob.properties.blobType === Models.BlobType.BlockBlob &&
            tier !== undefined) {
            copiedBlob.properties.accessTier = this.parseTier(tier);
     ...
     * @memberof LokiBlobMetadataStore
     */
    async getBlockList(context, account, container, blob, snapshot, isCommitted, leaseAccessConditions, modifiedAccessConditions) {
